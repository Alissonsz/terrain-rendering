...
const float levelOfDetail = 512;
const float maxLodDistance = 8;
...
void main () {
  ...
  if (gl_InvocationID == 0) {
    ...
    vec3 bTriangle = (gl_in[0].gl_Position.xyz + gl_in[1].gl_Position.xyz + gl_in[2].gl_Position.xyz)/3;
    TessLevelInner = LOD(bTriangle, viewPos);
