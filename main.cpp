...
void InputProcess(){
	...
	while(SDL_PollEvent(&event)){
		...
		if(SDL_GetMouseState(&x, &y) & SDL_BUTTON(SDL_BUTTON_LEFT)){
			...
			camera.ProcessMouseMovement(x - lastx, lasty - y, 1);
			lastx = x;
			lasty = y;
		} else {
			lastx = x;
...
void createNormalMap(uint16_t* heightMap, int width){
	std::cout << "Width at createNormalMap: " << width << std::endl;
...
//	float min = 15455, max = -800;
	...
	std::cout<<"Normal map created!"<<std::endl;
}
...
int main(int argc, char* args[]){
	...
	GLuint heightsTexture, texture2, normalTexture;
	...
	if (terrain == "montains") {
    	data1 = stbi_load_16("montainheights.png", &width, &height, &nrChannels, 0);
		TERRAIN_SIZE = 100.0f;
	} else if (terrain == "puget_sound") {
	...
	if (terrain == "montains") {
		uint16_t *data;
		data = stbi_load_16("montaincolors.png", &width, &height, &nrChannels, 0);
		std::cout << "Texture info:" << "Channels: " << nrChannels << "Width: " << width << "Height: " << height << std::endl;
	...
	ourTessShader.use();
	glUniform1i(glGetUniformLocation(ourTessShader.ID, "heightsTexture"), 0);
	...
	ourTessShader.setFloat("terrainSize", TERRAIN_SIZE);
